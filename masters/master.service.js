...
async function createBulk(req) {
  ...
  const url =
    "http://sfactivity.samparksmartshala.org/api/accounts/allclusters.json";
  http
...
async function createData(masterParam) {
  ...
  if (await State.findOne({ code: masterParam.state_code })) {
    ...
    await State.updateOne(
      { code: masterParam.state_code },
      { sf_state_id: masterParam.state_id }
    );
  } else {
    const stateParam = {
      name: masterParam.state_name,
      short_name: masterParam.state_short_name,
      code: masterParam.state_code,
      sf_state_id: masterParam.state_id,
    };
  ...
  if (
    await District.findOne({
      name: masterParam.district_name,
      state_id: state_id,
    })
  ) {
    ...
    await District.updateOne(
      { name: masterParam.district_name, state_id: state_id },
      { sf_district_id: masterParam.district_id }
    );
  } else {
    const districtParam = {
      name: masterParam.district_name,
      code: masterParam.district_code,
      sf_district_id: masterParam.district_id,
      state_id: state_id,
    };
  ...
  if (
    ...
    await Block.updateOne(
      { name: masterParam.block_name, district_id: district_id },
      { sf_block_id: masterParam.block_id }
    );
  } else {
    const blockParam = {
      name: masterParam.block_name,
      code: masterParam.block_code,
      sf_block_id: masterParam.block_id,
      district_id: district_id,
      state_id: state_id,
    };
  ...
  if (masterParam.cluster_name != "") {
    if (
      await Cluster.findOne({
        name: masterParam.cluster_name,
        block_id: block_id,
      })
    ) {
      await Cluster.updateOne(
        { name: masterParam.cluster_name, block_id: block_id },
        { sf_cluster_id: masterParam.cluster_id }
      );
      return "old";
    } else {
      const clusterParam = {
        name: masterParam.cluster_name,
        code: masterParam.cluster_code,
        sf_cluster_id: masterParam.cluster_id,
        block_id: block_id,
        district_id: district_id,
        state_id: state_id,
