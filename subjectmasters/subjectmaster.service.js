...
async function getAllWithDepartments() {
    const subject_masters = await Subjectmaster.find().sort({sort_order: 1}).select('-hash');
    const department_masters = await Departmentmaster.find().sort({sort_order: 1}).select('-hash');
    let subjects = {};
    let final_data = {};
    final_data['sss'] = [];
    final_data['sst'] = [];
    ...
    if (subject_masters && subject_masters.length  > 0) {
      subject_masters.forEach(subject => {
        ...
        if (subject['icon']) {
          subjects[subject["id"]]["icon"] = config.repositoryHost+subject['icon'];
        } else {
          subjects[subject["id"]]["icon"] = config.assetHost+subject['name'].toLowerCase()+".png";
        }
        if (subject['activeicon']) {
          subjects[subject["id"]]["icon_active"] = config.repositoryHost+subject['activeicon'];
        } else {
          subjects[subject["id"]]["icon_active"] = config.assetHost+"big-"+subject['name'].toLowerCase()+".png";
        }
        subjects[subject["id"]]['module'] = subject['module'];
        subjects[subject["id"]]['is_default'] = subject['is_default'];
        subjects[subject["id"]]['registration_name'] = (subject['registration_name']) ? subject['registration_name'] : subject['name'];
        subjects[subject["id"]]['for_registration'] = (subject['for_registration']) ? subject['for_registration'] : false;
    ...
    for (const [key, value] of Object.entries(subjects)) {
      ...
      if (final_data[value.module])
        final_data[value.module].push(value);
...
async function create(req) {
    ...
    // validate
    if (await Subjectmaster.findOne({ name: subjectmasterParam.name, module: subjectmasterParam.module })) {
...
async function update(id, req) {
    ...
    if (subjectmaster.module === subjectmasterParam.module && subjectmaster.name !== subjectmasterParam.name && await Subjectmaster.findOne({ name: subjectmasterParam.name, module: subjectmasterParam.module })) {
        throw 'Subject "' + subjectmasterParam.name + '" is already taken';
    }
    ...
    await subjectmaster.save();
    if (subjectmasterParam.is_default == true && subjectmaster.id != oldDefault.id)
    {
      await Subjectmaster.update({"_id" : oldDefault.id }, {is_default: false})
    }
}
...
async function _delete(id) {
    ...
    const subjectdepartment = await Departmentmaster.find({"subjects" : id}).countDocuments();
