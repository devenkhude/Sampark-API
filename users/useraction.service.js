...
async function getLeaderShip(req) {
  ...
  if (req.query.user) {
    ...
    let userType = "";
    if (
      curUser["usertype"] == "govt teacher" ||
      curUser["usertype"] == "teacher"
    )
      userType = "teacher";
    else if (curUser["usertype"] == "parent") userType = "parent";
    ...
    const mastercertificatess = await Certificate.find({
      is_active: true,
    }).sort({ subject: -1 });
    ...
    tempbadge["path"] =
      apk_version < 4.8 ? "/welcome.png" : config.assetHost + "welcome.png";
    tempbadge["is_active"] = true;
    tempbadge["expiry"] = welcomedate;
    tempbadge["description"] =
      "अरे वाह! आपने सीखने की दिशा में पहला कदम उठा लिया है! बैठक समुदाय इस पदक के साथ आपका स्वागत करता है।\n\nYou have taken the first step to learning! Baithak community welcomes you with this badge.";
    ...
    for (const mastercertificate of mastercertificatess) {
      ...
      tempcertificate["path"] =
        apk_version < 4.8
          ? mastercertificate["path"]
          : config.assetHost + mastercertificate["path"].replace("/", "");
    ...
    const classes = await Departmentmaster.find(
      { module: "sss" },
      { name: 1, id: 1, subjects: 1 }
    );
    let total_lessons = 0;
    for (const classdept of classes) {
      ...
      for (const classsubjectid of classdept.subjects) {
        ...
        if (subject_names[classsubjectid]) {
          ...
          class_subject_progress[classkey][subjectkey]["name"] =
            subject_names[classsubjectid]["name"];
          ...
          await Lesson.find({
            ...
            department: classdept.id,
            section: "sss",
          }).then((lessons) => {
            for (const lesson of lessons) {
              class_subject_progress[classkey]["lessons"] =
                class_subject_progress[classkey]["lessons"] + 1;
              total_lessons = total_lessons + 1;
              class_subject_progress[classkey][subjectkey]["lessons"] =
                class_subject_progress[classkey][subjectkey]["lessons"] + 1;

              subject_names[classsubjectid]["lessons"] =
                subject_names[classsubjectid]["lessons"] + 1;
          ...
          await Lessonprogress.find(query_video).then((lesson_progress) => {
            for (const progress of lesson_progress) {
              class_subject_progress[classkey]["progress"] =
                class_subject_progress[classkey]["progress"] +
                progress["progress"];
              class_subject_progress[classkey][subjectkey]["progress"] =
                class_subject_progress[classkey][subjectkey]["progress"] +
                progress["progress"];
              subject_names[classsubjectid]["progress"] =
                subject_names[classsubjectid]["progress"] +
                progress["progress"];
            }
          });
          let subjectprogress =
            class_subject_progress[classkey][subjectkey]["progress"] /
            class_subject_progress[classkey][subjectkey]["lessons"];
          class_subject_progress[classkey][subjectkey]["progress_percent"] =
            Math.round((subjectprogress + Number.EPSILON) * 100) / 100;
        }
      }
      let classprogress =
        class_subject_progress[classkey]["progress"] /
        class_subject_progress[classkey]["lessons"];
      class_subject_progress[classkey]["progress_percent"] =
        Math.round((classprogress + Number.EPSILON) * 100) / 100;
    }

    for (const subject of subjects) {
      let subjectprogress =
        subject_names[subject.id]["progress"] /
        subject_names[subject.id]["lessons"];
      subject_names[subject.id]["progress_percent"] =
        Math.round((subjectprogress + Number.EPSILON) * 100) / 100;
    }
    ...
    const completedCourses = await Enrollment.distinct("course", {
      status: "completed",
      user: user,
    });
    ...
    for (const mastercertificate of mastercertificates) {
      ...
      if (mastercertificate["department"] && mastercertificate["subject"]) {
        clessons = await Lesson.find({
          department: mastercertificate["department"],
          subject: mastercertificate["subject"],
          section: "sss",
        });
        cert_progress =
          class_subject_progress[mastercertificate["department"]][
        ...
        cert_progress =
          subject_names[mastercertificate["subject"]]["progress_percent"];
        clessons = await Lesson.find({
          subject: mastercertificate["subject"],
          section: "sss",
        });
      ...
      for (const clesson of clessons) {
        ...
        certificate_baithak_video = certificate_baithak_video.concat(
          clesson.baithak_videos
        );
      }
      let duration = 0;
      ...
      for (const video of certificate_video) {
        if (video != "" && check_videos.indexOf(video.toString()) === -1) {
          ...
          let video_detail = await Video.findById(video)
            .populate("department", "name")
            .populate("subject", "name");
          duration =
            duration +
            (video_detail.duration_min * 60 + video_detail.duration_sec);
      ...
      mastercertificate["duration_watched"] = Math.ceil(
        duration_watched / (60 * 60)
      );
      mastercertificate["total_duration"] = parseInt(duration / (60 * 60));
      mastercertificate["duration_watched"] = parseInt(
        duration_watched / (60 * 60)
      );

      if (
        mastercertificate["total_duration"] -
          mastercertificate["duration_watched"] <=
          0 &&
        cert_progress == 100
      ) {
    ...
    if (Object.keys(certificates).length > 0) {
      const userexistingcertificates = await Usercertificate.distinct(
        "certificate",
        { user: user, certificate: { $in: Object.keys(certificates) } }
      );
    ...
    if (Object.keys(certificates).length > 0) {
      try {
        ...
        for (const certificate of Object.keys(certificates)) {
          const fileData = certificatesfiles.filter(
            (p) => p.id === certificate
          );
          ...
          if (fileData.length) {
            try {
              const certificateUrl =
                config.assetHost + fileData[0].filePath.replace("/", "");
              await downloadFileFromURL(
                certificateUrl,
                "../assets/",
                fileData[0].filePath.replace("/", "")
              );
            } catch (e) {
              console.log(e);
            }
        ...
        try {
          ...
          try {
            ...
            if (userUpdatedCertificates.length) {
              ...
              for (
                let cerificateIndex = 0;
                cerificateIndex < userUpdatedCertificates.length;
                cerificateIndex++
              ) {
                const filePathCertificate = path.resolve(
                  __dirname,
                  "../uploads/usercertificates/" +
                    userUpdatedCertificates[cerificateIndex].value
                );

                const file = fs.createReadStream(filePathCertificate);

                let updateResponse = await uploadStreamToS3(
                  file,
                  "usercertificates",
                  userUpdatedCertificates[cerificateIndex].value
                );
                ...
                // create the pdf
                let filePathCertificatePdf = filePathCertificate.replace(
                  ".png",
                  ".pdf"
                );
                ...
                await imagesToPdf(
                  [filePathCertificate],
                  filePathCertificatePdf
                );

                const pdfFile = fs.createReadStream(filePathCertificatePdf);
                const pdfFileName = userUpdatedCertificates[
                  cerificateIndex
                ].value.replace(".png", ".pdf");
                let updateResponsePdf = await uploadStreamToS3(
                  pdfFile,
                  "usercertificates",
                  pdfFileName
                );
                ...
                if (updateResponse && updateResponsePdf) {
                  ...
                  await Usercertificate.updateOne(
                    {
                      user: user,
                      certificate: userUpdatedCertificates[cerificateIndex].key,
                    },
                    {
                      $set: {
                        path: updateResponse.key,
    ...
    if (usercertificates.length > 0 || Object.keys(certificates).length > 0) {
      for (const usercertificate of usercertificates) {
        if (usercertificate["certificate"] != undefined) {
          certificates[usercertificate["certificate"]] =
            apk_version < 4.8
              ? "/" + usercertificate["path"]
              : config.aws_s3_url + usercertificate["path"] + "";
        }
        if (usercertificate["course"] != undefined) {
          let courseCertificateIndex = existingCourses.indexOf(
            usercertificate["course"].toString()
          );
      ...
      for (const mastercertificate of mastercertificates) {
        if (certificates.hasOwnProperty(mastercertificate["_id"])) {
          ...
          let url = certificates[mastercertificate["_id"]].replace(
            ".png",
            ".pdf"
          );
          let fileName = "";
          ...
          if (res && res.statusCode == 404) {
            ...
            mastercertificate["pdfpath"] = certificates[
              mastercertificate["_id"]
            ]
              ? certificates[mastercertificate["_id"]].replace(".png", ".pdf")
              : "";
          }
          //  mastercertificate['pdfpath'] = certificates[mastercertificate['_id']].replace(".png",".pdf");
          mastercertificate["message"] = "You have achived this! Brag about it";
        } else if (
          mastercertificate.hasOwnProperty("type") &&
          mastercertificate["type"] == "elearning"
        ) {
          mastercertificate["is_active"] = true;
        } else {
          mastercertificate["pdfpath"] = "";
...
async function sendcertificate(userParam) {
  ...
  const usercertificate = await Usercertificate.find({
    user: user,
  ...
  if (usercertificate.length == 1 && usercertificate[0].path !== "") {
    ...
    try {
      ...
      const filePath = await downloadFileFromURL(
        fileUrl,
        "../uploads/usercertificates/",
        userdetails.id + "_" + certificatedetails.id + ".png"
      );
      await imagesToPdf(
        [config.uploadPath + usercertificate[0].path],
        config.uploadPath +
          "usercertificates/" +
          userdetails.id +
          "_" +
    ...
    if (certificatedetails.subject && certificatedetails.department) {
      ...
      emailtextbody =
        ...
        " जी, %0A%0Aबधाई हो! सम्पर्क फाउंडेशन की तरफ से आपको सर्टिफिकेट ऑफ़ कम्पलीशन से सम्मानित किया जाता है। %0A%0Aहमें गर्व है की आपके जैसे जागरूक और जिज्ञासु शिक्षक बैठक समुदाय का हिस्सा हैं।%0Aइसी तरह अपनी लर्निंग यात्रा को चलने दीजिये। %0A%0A%0Aसादर, बैठक समुदाय";
      emailhtmlbody =
        "प्रिय " +
        userdetails.firstName +
        " जी, <br><br>बधाई हो! सम्पर्क फाउंडेशन की तरफ से आपको सर्टिफिकेट ऑफ़ कम्पलीशन से सम्मानित किया जाता है। <br><br>हमें गर्व है की आपके जैसे जागरूक और जिज्ञासु शिक्षक बैठक समुदाय का हिस्सा हैं।<br>इसी तरह अपनी लर्निंग यात्रा को चलने दीजिये। <br><br><br>सादर,<br>बैठक समुदाय";
    ...
    const filePathCertificateImage = path.resolve(
      __dirname,
      "../uploads/usercertificates/" +
        userdetails.id +
        "_" +
    ...
    const filePathCertificatePdf = path.resolve(
      __dirname,
      "../uploads/usercertificates/" +
        userdetails.id +
        "_" +
...
async function getById(id) {
  ...
  if (user !== "") {
    const userlikes = await Streamlike.find({ user: user, stream: id }).select(
      "stream is_liked"
    );
  ...
  stream["thumbnail"] =
    "https://img.youtube.com/vi/" + stream["stream_code"] + "/hqdefault.jpg"; //to be remove
...
async function getScertUploads() {
  userVideos = await Userscertvideo.find().populate(
    "user",
    "firstName lastName"
  );
  streamList = [];
...
async function uploadscert(req) {
  ...
  if (uploadFolder != "") {
    ...
    if (reqbody.uploadtype == "image") {
      const userimagescertstream = new Imagescertstream({
        name: "",
        sort_order: 0,
        thumbnail: uploadData.Key,
        description: postDescription,
        author: reqbody.user,
        createdBy: reqbody.user,
        updatedBy: reqbody.user,
        is_shareable: true,
      });
      ...
      const userpdfscertstream = new Pdfscertstream({
        name: "",
        sort_order: 0,
        pdf: uploadData.Key,
        description: postDescription,
      ...
      const userscertupload = new Userscertvideo({
        path: "userscertvideos/" + sampleFile.name,
        description: postDescription,
        user: reqbody.user,
      });
    ...
    const usertextscertstream = new Textscertstream({
      name: "",
      sort_order: 0,
...
async function postaction(userParam) {
  ...
  if (stream.item_type == "video" && stream.is_sampark_didi_post == false) {
    ...
    if (
      uservideostream &&
      uservideostream.youtube_code != "" &&
      uservideostream.youtube_code != null
    )
  ...
  if (dir != "") {
    ...
    if (data.body.uploadtype == "image") {
      ...
      if (data.body.editpost) {
        ...
        if (
          data.body.hashtags &&
          data.body.hashtags != undefined &&
          data.body.hashtags.length > 0
        )
          streamData.hashtags = data.body.hashtags;
        else streamData.hashtags = [];
        ...
        if (upldaoDataList.length) {
          ...
          userimagestream = new Imagestream({
            name: "",
            sort_order: 0,
        ...
        if (curUserCluster !== "") streamData.cluster = curUserCluster;
        if (curUserBlock !== "") streamData.block = curUserBlock;
        ...
        else streamData.is_sampark_didi_post = false;
      ...
      if (
        !data.body.editpost &&
        data.body.user != "" &&
        (streamData.postOn == "Baithak" || streamData.postOn == "Both")
      )
      ...
      if (data.body.editpost) {
        ...
        if (
          data.body.isVideoEmpty &&
          (data.body.isVideoEmpty === true || data.body.isVideoEmpty === "true")
        ) {
          ...
          const deleteReponse = await deleteOnYouTube(
            uservideostream.youtube_code
          );
          if (deleteReponse && deleteReponse.status === 204) {
            ...
            if (uploadResponse["status"] == 200) {
              ...
              let duration =
                uploadResponse["data"]["contentDetails"]["duration"];
              let arr = duration.split("M");
              if (arr && arr.length == 2) {
              ...
              streamData.diseCode = data.body.diseCode
                ? data.body.diseCode
                : "";
              streamData.pincode = data.body.pincode ? data.body.pincode : "";
              // if (data.body.userType == 'govt teacher')
              streamData.postOn = data.body.postOn
                ? data.body.postOn
                : "Baithak";
    ...
    if (data.body.editpost) {
      ...
      const { description, user, reasonToShare, resourceType, resourceLink } =
        data.body;
      usertextstream = new Textstream({
        ...
        author: user,
        reasonToShare:
          reasonToShare != "" && reasonToShare != null ? reasonToShare : "",
        resourceType:
          resourceType != "" && resourceType != null ? resourceType : "",
        resourceLink:
          resourceLink != "" && resourceLink != null ? resourceLink : "",
        createdBy: user,
        updatedBy: user,
        is_shareable: true,
      });
...
async function deleteComment(data) {
  try {
    ...
    if (module_name == "scert") {
      ...
      let scertstream = await Scertstream.find({
        item_type: item_type,
        item_id: stream.id,
      });
...
async function comments(req) {
  ...
  try {
    const { module: module_name = "", item_type = "", item = "" } = req;
    const user_comments = [];
    let comments = "";

    const query = buildQuery(item_type, item);

    switch (item_type) {
      case "image":
        comments = await getImageComments(module_name, query);
        break;
      case "video":
        comments = await getVideoComments(module_name, query);
        break;
      case "pdf":
        comments = await getPDFComments(query);
        break;
      case "text":
        comments = await getTextComments(module_name, query);
        break;
      case "sssvideo":
        comments = await getSssVideoComments(query);
        break;
      case "audiotextbook":
        comments = await getAudioTextbookComments(query);
        break;
    }

    user_comments = await processComments(comments);
...
function buildQuery(item_type, item) {
  const query = {};
  query[item_type] = item;
  return query;
}

async function getImageComments(module_name, query) {
  const model = module_name === "scert" ? Scertimagecomment : Imagecomment;
  return model
    .find(query)
    .populate("user", [
      "fullName",
...
async function getVideoComments(module_name, query) {
  const model = module_name === "scert" ? Scertvideocomment : Videocomment;
  return model
    .find(query)
    .populate("user", [
      "fullName",
...
async function getPDFComments(query) {
  return Scertpdfcomment.find(query)
    .populate("user", [
      "fullName",
      "firstName",
      "lastName",
...
async function getTextComments(module_name, query) {
  const model = module_name ? Scerttextcomment : Textcomment;
  return model
    .find(query)
    .populate("user", [
      "fullName",
...
async function getSssVideoComments(query) {
  return Sssvideocomment.find(query)
    .populate("user", [
      "fullName",
      "firstName",
      "lastName",
...
async function getAudioTextbookComments(query) {
  return Audiotextbookcomment.find(query)
    .populate("user", [
      "fullName",
      "firstName",
      "lastName",
  ...
  for (const comment of comments) {
    if (comment.user) {
      const user_comment = {
        ...
        caricatures: comment.caricatures
          ? comment.caricatures.map((e) => config.repositoryHost + e)
          : [],
        name: comment.user.fullName
          ? titleize(comment.user.fullName)
          : titleize(comment.user.firstName),
        ...
        image:
          comment.user.image !== "" && comment.user.image !== null
            ? config.repositoryHost + comment.user.image
            : config.user_image,
        replies: await getSubComments(comment._id).catch((err) => []),
      };
...
async function getSubComments(commentId) {
  ...
  try {
    ...
    const subCommentData = await Subcomment.find(query)
      .populate("user", [
        "fullName",
        "firstName",
        "lastName",
    ...
    const subComments = subCommentData.map((comment) => {
      return {
        comment: comment.comment,
        comment_id: comment._id,
        caricatures: comment.caricatures.length
          ? comment.caricatures.map((e) => config.repositoryHost + e)
          : [],
        name: comment.user.fullName
          ? titleize(comment.user.fullName)
...
async function replyOnComment(req) {
  ...
  if (comment != "" && comment != null) {
    if (
      ...
      item_type == "video" ||
      item_type == "sssvideo" ||
      item_type == "audiotextbook"
    ) {
      const reply = new Subcomment({
        comment: comment,
        user: user,
      ...
      if (await reply.save()) {
        if (item_type == "image") {
          ...
          const stream = await Stream.find({
            item_type: "sssvideo",
            item_id: sssvideoComment.video,
          });
          await create_notification(stream[0], sssvideoComment, "reply", user);
        }

        return { success: true };
      } else throw "Failed to save in DB";
    } else throw "Please send correct request";
  } else throw "Please send correct request";
}

async function checkcommentexistance(
  item_type,
  comment,
  user,
  item,
  ...
  if (item_type == "image") {
    usercommentcount = await Imagecomment.countDocuments({
      comment: comment,
      user: user,
      image: item,
      caricatures: caricatures,
    ...
    usercommentcount = await Videocomment.countDocuments({
      comment: comment,
      user: user,
      video: item,
      caricatures: caricatures,
    ...
    usercommentcount = await Textcomment.countDocuments({
      comment: comment,
      user: user,
      text: item,
      caricatures: caricatures,
    ...
    usercommentcount = await Sssvideocomment.countDocuments({
      comment: comment,
      user: user,
      video: item,
      caricatures: caricatures,
  ...
  if (usercommentcount > 0) {
    if (item_type == "image") {
      userComment = await Imagecomment.findOne({
        comment: comment,
        user: user,
        image: item,
        caricatures: caricatures,
      ...
      userComment = await Videocomment.findOne({
        comment: comment,
        user: user,
        video: item,
        caricatures: caricatures,
      ...
      userComment = await Textcomment.findOne({
        comment: comment,
        user: user,
        text: item,
        caricatures: caricatures,
      ...
      userComment = await Sssvideocomment.findOne({
        comment: comment,
        user: user,
        video: item,
        caricatures: caricatures,
...
async function comment(req) {
  try {
    ...
    let isCommentExist = await checkcommentexistance(
      item_type,
      comment,
      user,
      item,
    ...
    if (module == "scert") {
      if (item_type == "image") {
        const usercomment = new Scertimagecomment({
          comment: comment,
          user: user,
          image: item,
        });
        ...
        if (commentData) {
          const usercomments = await Scertimagecomment.countDocuments({
            image: item,
          });
          ...
          const scertstream = await Scertstream.find({
            item_type: "image",
            item_id: item,
          });
          sParam = {};
        ...
        const usercomment = new Scertvideocomment({
          comment: comment,
          user: user,
          video: item,
        });
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Scertvideocomment.countDocuments({
            video: item,
          });
        ...
        const usercomment = new Scerttextcomment({
          comment: comment,
          user: user,
          text: item,
        });
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Scerttextcomment.countDocuments({
            text: item,
          });
        ...
        const usercomment = new Scertpdfcomment({
          comment: comment,
          user: user,
          pdf: item,
        });
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Scertpdfcomment.countDocuments({
            pdf: item,
          });
      ...
      if (item_type == "image") {
        const usercomment = new Imagecomment({
          comment: comment,
          user: user,
          image: item,
          caricatures: caricatures,
        ...
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Imagecomment.countDocuments({
            image: item,
          });
        ...
        const usercomment = new Videocomment({
          comment: comment,
          user: user,
          video: item,
          caricatures: caricatures,
        ...
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Videocomment.countDocuments({
            video: item,
          });
        ...
        const usercomment = new Textcomment({
          comment: comment,
          user: user,
          text: item,
          caricatures: caricatures,
        ...
        const usercomment = new Sssvideocomment({
          comment: comment,
          user: user,
          video: item,
          caricatures: caricatures,
        ...
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Sssvideocomment.countDocuments({
            video: item,
          });
        ...
        const usercomment = new Audiotextbookcomment({
          comment: comment,
          user: user,
          audiotextbook: item,
          caricatures: caricatures,
        ...
        let commentData = await usercomment.save();
        if (commentData) {
          const usercomments = await Audiotextbookcomment.countDocuments({
            audiotextbook: item,
          });
...
async function likescertstream(req) {
  ...
  if (like == true || like == "true") {
    ...
    if (item_type == "image") {
      const userlike = new Scertimagelike({
        is_liked: like,
        device_id: device_id,
        image: item,
      });
      ...
      if (await userlike.save()) {
        ...
        Object.assign(imagestream, streamParam);
        await imagestream.save();
        const stream = await Scertstream.find({
          item_type: "image",
          item_id: item,
        });
        sParam = {};
      ...
      const userlike = new Scertvideolike({
        is_liked: like,
        device_id: device_id,
        video: item,
      });
      ...
      const userlike = new Scerttextlike({
        is_liked: like,
        device_id: device_id,
        text: item,
      });
      ...
      const userlike = new Scertpdflike({
        is_liked: like,
        device_id: device_id,
        pdf: item,
      });
    ...
    if (like == true || like == "true") {
      ...
      if (item_type == "image") {
        ...
        if (checkuserlike) {
          ...
          const userlike = new Sssvideolike({
            is_liked: like,
            device_id: device_id,
            video: item,
          });
          ...
          if (await userlike.save()) {
            const userlikes = await Sssvideolike.countDocuments({
              video: item,
            });
            const sssvideo = await Video.findById(item);
            // let viewCountUpdate = sssvideo['viewcount'];
        ...
        if (checkuserlike) {
          ...
          const userlike = new Audiotextbooklike({
            is_liked: like,
            device_id: device_id,
            audiotextbook: item,
          });
          ...
          if (await userlike.save()) {
            const userlikes = await Audiotextbooklike.countDocuments({
              audiotextbook: item,
            });
            const audiotextbook = await Audiotextbook.findById(item);
            // let viewCountUpdate = sssvideo['viewcount'];
  ...
  try {
    ...
    if (durationcheck > 0) {
      ...
      await Stream.updateOne(
        { item_type: item_type, item_id: item },
        { $inc: { viewcount: 1 }, updatedDate: currentISTDate.toISOString() }
      );
      ...
      if (item_type === "video") {
        ...
        if (video) {
          ...
          const userplayed = new Videoplayed({
            duration: duration,
            video: item,
            device_id: device_id,
          });
          if (user) {
            ...
            let duration_in_sec =
              video["duration_min"] * 60 + video["duration_sec"];
            let duration_for_pass =
              (parseFloat(config.video_passing_percentage) / 100) *
              parseFloat(duration_in_sec);
            if (duration >= duration_for_pass) {
              try {
                await Videoviewed.updateOne(
                  { video: item, user: user },
                  { $set: { video: item, user: user } },
                  { upsert: true }
                );
                const checklessons = await Lesson.find({
                  baithak_videos: item,
                });
        ...
        if (audio) {
          ...
          const userplayed = new Audioplayed({
            duration: duration,
            audio: item,
            device_id: device_id,
          });
          ...
          try {
            if (user) {
              userplayed.user = user;
              let duration_in_sec =
                audio["duration_min"] * 60 + audio["duration_sec"];
              let duration_for_pass =
                (parseFloat(config.video_passing_percentage) / 100) *
                parseFloat(duration_in_sec);
              if (duration >= duration_for_pass) {
                try {
                  await Audioviewed.updateOne(
                    { audio: item, user: user },
                    { $set: { audio: item, user: user } },
                    { upsert: true }
                  );
        ...
        if (video) {
          ...
          const userplayed = new Sssvideoplayed({
            duration: duration,
            video: item,
            device_id: device_id,
          });
          if (user) {
            ...
            if (duration >= duration_for_pass) {
              try {
                await Sssvideoviewed.updateOne(
                  { video: item, user: user },
                  { $set: { video: item, user: user } },
                  { upsert: true }
                );
        ...
        const audiotextbook = await Audiotextbook.findById(item);
        if (audiotextbook) {
          await Audiotextbook.updateOne(
            { _id: item },
            { $inc: { viewcount: 1 } }
          ); //commented as viewcount will be increase from streams
          const userplayed = new Audiotextbookplayed({
            duration: duration,
            audiotextbook: item,
            device_id: device_id,
          });
          if (user) {
            userplayed.user = user;
            let duration_in_sec =
              audiotextbook["duration_min"] * 60 +
              audiotextbook["duration_sec"];
            let duration_for_pass =
              (parseFloat(config.video_passing_percentage) / 100) *
              parseFloat(duration_in_sec);
            if (duration >= duration_for_pass) {
              try {
                await Audiotextbookviewed.updateOne(
                  { audiotextbook: item, user: user },
                  { $set: { audiotextbook: item, user: user } },
                  { upsert: true }
                );
  ...
  try {
    ...
    if (user != "") {
      ...
      if (
        ...
        item_type == "quiz" ||
        item_type == "progress_chart" ||
        item_type == "lesson_plan" ||
        item_type == "project_work"
      ) {
        const documentviewed = new Documentviewed({
          document: item,
          user: user,
        });
        try {
          await documentviewed.save();
          await Documentviewed.updateOne(
            { document: item, user: user },
        ...
        const scertsolutionviewed = new Scertsolutionviewed({
          scertsolution: item,
          user: user,
        });
        try {
          await scertsolutionviewed.save();
          await Scertsolutionviewed.updateOne(
            { scertsolution: item, user: user },
            { $set: { device_id: device_id } }
          );
        ...
        const sssAssessmentViewed = new Sssassessmentviewed({
          user: user,
          assessment: item,
        });
        try {
        ...
        const englishspeechViewed = new Englishspeechviewed({
          user: user,
          englishspeechId: item,
        });
        try {
...
async function lessonunlocked(request_body) {
  ...
  try {
    ...
    if (checklesson && checklesson["_id"] == item) {
      ...
      if (user)
        isUnlockedlesson = await Unlockedlesson.countDocuments({
          lesson: item,
          user: user,
        });
      else
      ...
      if (isUnlockedlesson === 1) {
        ...
        if (user) unlockedlesson.user = user;
        else unlockedlesson.device_id = device_id;
...
async function followunfollow(userParam) {
  ...
  const userfollower = await Userfollower.find({
    follower: follower,
    following: following,
  });
  ...
  if (action == "F") {
    if (!userfollower.length) {
      ...
      await Userfollower.updateOne(
        { follower: follower, following: following, action: "U" },
        { $set: { action: "F", createdBy: follower, modifiedBy: follower } },
        { upsert: false }
      );
      const followerDetail = await User.findById(follower);
      const followingDetail = await User.findById(following);
    ...
    if (
      userfollower &&
      userfollower.length &&
      userfollower[0]["action"] == "F"
    ) {
      await Userfollower.updateOne(
        { follower: follower, following: following, action: "F" },
        { $set: { action: "U", createdBy: follower, modifiedBy: follower } },
        { upsert: false }
      );
    } else {
...
async function followers(userParam) {
  ...
  var userdetails = await User.find({ _id: { $in: arrFollowerIds } }).select(
  ...
  if (userdetails && userdetails.length) {
    for (let index = 0; index < userdetails.length; index++) {
      ...
      followerObj["profilePic"] =
        element.image != "" && element.image != null
          ? config.repositoryHost + element.image
          : config.user_image;
...
async function getCountDetails(userId, user, arrUserDetails) {
  arrUserDetails["completedCoursesCnt"] = user.noOfCompletedCourses
    ? user.noOfCompletedCourses
    : 0;
  arrUserDetails["goldBadgesCnt"] = await Userbadge.countDocuments({
    user: new objectId(userId),
    badge: "gold",
  });
  arrUserDetails["silverBadgesCnt"] = await Userbadge.countDocuments({
    user: new objectId(userId),
    badge: "silver",
  });
  arrUserDetails["bronzeBadgesCnt"] = await Userbadge.countDocuments({
    user: new objectId(userId),
    badge: "bronze",
  });
  arrUserDetails["learningPts"] = 50;
  arrUserDetails["followersCnt"] = await Userfollower.countDocuments({
...
async function getStreamDetails(userId, arrUserDetails) {
  ...
  arrUserDetails["viewsCnt"] = streamRecord.length
    ? streamRecord[0]["totalViews"]
    : 0;
  arrUserDetails["postsCnt"] = streamRecord.length
    ? streamRecord[0]["postsCnt"]
    : 0;
  return arrUserDetails;
}
...
async function getOtherUserRank(userdetails, userId, arrUserDetails) {
  ...
  const collectionName =
    userType == "govt teacher" ? Teacherranks : Parentranks;
...
async function otherProfile(userParam) {
  try {
    ...
    // Get the follow status w.r.t. other profile user clicked
    ...
    const userdetails = await User.findOne(
      ...
      "id firstName lastName image state noOfCompletedCourses usertype district registrationDate diseCode schoolName totalResources"
    );
    ...
    let arrUserDetails = {
      followStatus: (userfollower && userfollower.action) || null,
      id: userId,
      firstName: userdetails.firstName,
      lastName: userdetails.lastName,
      profilePic:
        userdetails.image !== "" || userdetails.image !== null
          ? config.repositoryHost + userdetails.image
          : config.user_image,
      registrationDate: userdetails.registrationDate,
      totalResources: userdetails.totalResources,
    };
    ...
    if (userdetails.schoolName !== "") {
      ...
      const school = await School.findOne(
        { diseCode: userdetails.diseCode },
        "schoolName"
      );
      arrUserDetails.schoolName = (school && school.schoolName) || "";
    }

    if (userdetails.state) {
      arrUserDetails.state = userdetails.state;
      const userstate = await State.findById(userdetails.state, "name");
      arrUserDetails.stateName = (userstate && userstate.name) || "My State";
    }
    ...
    // Get Other user's rank in district
    arrUserDetails = await getOtherUserRank(
      userdetails,
      userId,
      arrUserDetails
    );
...
async function getSparkleDifference(
  ...
  try {
    ...
    let getAboveRange = await collectionName
      .find({ district: district, rank: { $lt: range } }, { grandTotal: 1 })
      .sort({ grandTotal: 1 })
      .limit(1);
...
async function recordPostShare(userParam) {
  ...
  if (
    ...
    const postShareData = await PostShare.find({
      userId: userId,
      streamId: streamId,
    });
    ...
    if (!postShareData.length) {
      ...
      if (uniqueShareCount && uniqueShareCount == 100) {
        let checkUserBadge = await Userbadge.find({
          user: userId,
          badge: "unique-sharing-100",
        });
        if (checkUserBadge.length == 0) {
          ...
          let centuryBadge = new Userbadge({
            user: userId,
            badge: "unique-sharing-100",
            type: "uniq-sharing",
        ...
        if (checkUserBadge.length == 0) {
          ...
          let doubleCenturyBadge = new Userbadge({
            user: userId,
            badge: "unique-sharing-200",
            type: "uniq-sharing",
            expiryDate: expiry_date,
        ...
        if (checkUserBadge.length == 0) {
          ...
          let tripleCenturyBadge = new Userbadge({
            user: userId,
            badge: "unique-sharing-300",
            type: "uniq-sharing",
            expiryDate: expiry_date,
        ...
        if (checkUserBadge.length == 0) {
          ...
          let goldenBatBadge = new Userbadge({
            user: userId,
            badge: "unique-sharing-500",
            type: "uniq-sharing",
            expiryDate: expiry_date,
    ...
    try {
      ...
      try {
        try {
          ...
          let updateResponseImage = await uploadStreamToS3(
            certFile,
            "usercertificates",
            user + "_" + course + ".png"
          );
