...
async function last_active_time(userid, device_id, action_performed, apk_version, fcm_token) {
  if (device_id != "") {
    if (userid == "") {
      check_user_device = await Userdevice.findOne({ device_id: device_id });
    } else {
      check_user_device = await Userdevice.findOne({ device_id: device_id, user: userid });
    }
    if (check_user_device != null && check_user_device.device_id == device_id) {
      ...
      user_device = new Userdevice({ device_id: device_id, apk_version: apk_version, action_performed: action_performed });
      if (userid != "") {
    ...
    if (userid !== "") {
      ...
      if (fcm_token != "" && fcm_token != "undefined" && fcm_token !== undefined) {
        await User.updateOne({ "_id": userid }, { "$set": { "fcm_token": fcm_token } })
      }
      checkattendance = await Userattendance.find({ "user": userid, "attendance_date": today })
      if (checkattendance.length == 0) {
        markattendance = new Userattendance({ "user": userid, "attendance_date": today });
...
async function getStateDistrict(user) {
  ...
  if (user.state) {
    ...
    stateshortname = (userstate && userstate.name) ? userstate.short_name : "USNA"
  ...
  if (user.district) {
    ...
    districtname = (userdistrict && userdistrict.name) ? userdistrict.name : "My District"
    districtname = districtname.toLowerCase()
      .split(' ')
      .map((s) => s.charAt(0).toUpperCase() + s.substring(1))
      .join(' ');
  }
  if (user.usertype == "spark" || user.usertype == "parent" || user.usertype == "govt teacher") {
    baithakUpload = true
  }
  ...
  helpline_no = (user.state && config.covid_helpline_nos[user.state.toString()]) ? config.covid_helpline_nos[user.state.toString()] : ""
  return [statename, districtname, baithakUpload, certificateAvailable, ncertname, district, block, helpline_no];
}
  ...
  try {
    if (id) {
      ...
      if (user) {
        ...
        const profile_pic = (user.image != "" && user.image != null) ? config.repositoryHost + user.image : config.user_image;
        return {
          id,
          ...userWithoutHash,
          token,
...
async function getAuthors(author) {
  if (author) {
    if (author === "sampark") {
      ...
      return await User.find({ "_id": { "$in": temp_sampark_didi_ids } }).select('fullName phone_number');
    } else {
...
async function changePassword(userParam) {
  ...
  try {
    const checkuser = await User.findOne({ phone_number: userParam.phone_number });
    if (checkuser) {
      // hash password
      let password = userParam.password;
      let userHash = bcrypt.hashSync(password, 10);
      await User.updateOne({ phone_number: userParam.phone_number }, { "$set": { hash: userHash } })
...
async function create(userParam) {
  ...
  try {
    ...
    if (checkuser) {
      defer.reject('Phone Number "' + userParam.phone_number + '" is already registered');
    } else {
      ...
      const user1 = await User.find({ phone_number: userParam.phone_number, is_new: true })
      ...
      if (!userUParam.state || userUParam.state == "" || userUParam.state == "null") {
      ...
      if (userParam.lastName)
        userUParam.fullName = userParam.firstName.trim() + " " + userParam.lastName.trim();
      else
        userUParam.fullName = userParam.firstName.trim();
      ...
      if (user) {
        ...
        if (userParam.hasOwnProperty("diseCode") && userParam.diseCode.trim() != "") {
        ...
        // save user
        await user.save();
        const user2 = await User.findOne({ phone_number: userParam.phone_number });
        if (user2) {
          const { _id, hash, ...userWithoutHash } = user2.toObject();
          const id = _id;
          const token = jwt.sign({ sub: user2.id }, config.secret);
          const profile_pic = (user2.hasOwnProperty('image') && user2.image != "" && user2.image != null) ? config.repositoryHost + user2.image : config.user_image;
          const is_usertype_exists = (user2.usertype == "") ? false : true
          const is_registered = (user2.firstName == "" || user2.pincode == "") ? false : true
...
function sendsms(sms, mobile) {
  if (config.environment == "PRODUCTION" && config.vip_numbers.indexOf(mobile) === -1) {
    sms = encodeURI(sms)
    ...
    let url = "https://japi.instaalerts.zone/httpapi/QueryStringReceiver?ver=1.0&encrpt=0&key=EWDpbVV7VlPZCj6yGEWtVw==&send=SMPRKF&dest=" + mobile + "&text=" + sms;
...
async function login(userParam) {
  ...
  try {
    ...
    if (!user) {
      ...
      let message = "Your one time password for Sampark Smart Shala is " + newUser.otp + ". Verify to sign in!";
      sendsms(message, userParam.phone_number);
      // save user
      await newUser.save();
      //return await User.findOne({ phone_number: userParam.phone_number }).select('-hash');
      ...
      // copy userParam properties to user
      Object.assign(user, userParam);
      if (user.phone_number == "1111111111" || user.phone_number == "9810184062") {
        user.otp = "1111";
      } else {
        user.otp = createotp(4, userParam.phone_number, user.otp);
        let message = "Your one time password for Sampark Smart Shala is " + user.otp + ". Verify to sign in!"; //+" is your OTP for Smart Shala. This is usable once and please DO NOT share with anyone.";
        sendsms(message, userParam.phone_number);
      }
      await user.save();
      //return await User.findOne({ phone_number: userParam.phone_number }).select('-hash');
    ...
    if (user1) {
      ...
      const profile_pic = (user1.hasOwnProperty('image') && user1.image != "" && user1.image != null) ? config.repositoryHost + user1.image : config.user_image;
      const is_usertype_exists = (user1.usertype == "") ? false : true
      const is_registered = (user1.firstName == "" || user1.pincode == "") ? false : true
      stateDistrict = await getStateDistrict(user1);
      statename = stateDistrict[0];
      districtname = stateDistrict[1];
      baithakupload = stateDistrict[2];
...
async function update_usertype(userParam) {
  ...
  try {
    ...
    if (user1) {
      const { _id, hash, ...userWithoutHash } = user1.toObject();
      const id = _id;
      const profile_pic = (user1.image != "" && user1.image != null) ? config.repositoryHost + user1.image : config.user_image;
      const token = jwt.sign({ sub: user1.id }, config.secret);
      const is_usertype_exists = (user1.usertype == "") ? false : true
      const is_registered = (user1.firstName == "" || user1.pincode == "") ? false : true
      stateDistrict = await getStateDistrict(user1);
...
async function update(id, userParam) {
  ...
  try {
    ...
    // validate
    if (!user) throw 'User not found';
    if (user.phone_number !== userParam.phone_number && await User.findOne({ phone_number: userParam.phone_number })) {
    ...
    if (userParam.schoolCode) {
      ...
      if (school) {
        ...
        const schoolName = (school.schoolName && school.schoolName !== "") ? school.schoolName : "";
        await User.updateOne({ "_id": id }, { "$set": { "schoolName": schoolName, "schoolId": schoolId } });
      }
      // user.schoolId = schoolId;
      // user.schoolName = schoolName;
    }
    ...
    if (!userParam.state || userParam.state == "" || userParam.state == "null") {
    ...
    userdistrictname = ""
    if (userParam.lastName)
      userParam.fullName = userParam.firstName.trim() + " " + userParam.lastName.trim();
    else
      userParam.fullName = userParam.firstName.trim();
...
async function update_fcmtoken(id, userParam) {
  ...
  try {
    ...
    if (user) {
      ...
      if ((today - registrationDate) / (24 * 3600 * 1000) <= config.landing_page_days) {
        ...
        if (sparkleUser) {
          ...
          let limit10 = (cntTopTeachers > 10) ? parseInt((cntTopTeachers * 10) / 100) : cntTopTeachers;

          const top10Teachers = await Topteachers.find(query).sort({ "total": -1 }).limit(limit10);
          ...
          let limit20 = (cntTopTeachers > 20) ? parseInt((cntTopTeachers * 20) / 100) : cntTopTeachers;

          const top20Teachers = await Topteachers.find(query).sort({ "total": -1 }).skip(limit10).limit(limit20);
          ...
          let limit30 = (cntTopTeachers > 30) ? parseInt((cntTopTeachers * 30) / 100) : cntTopTeachers;

          const top30Teachers = await Topteachers.find(query).sort({ "total": -1 }).skip(limit20).limit(limit30);
          ...
          let limit50 = (cntTopTeachers > 50) ? parseInt((cntTopTeachers * 50) / 100) : cntTopTeachers;

          const top50Teachers = await Topteachers.find(query).sort({ "total": -1 }).skip(limit30).limit(limit50);
...
async function avasar(userParam) {
  ...
  try {
    ...
    let parent = {};
    const checkuserphone = await User.findOne({ phone_number: userParam.phone_number });
    const checkstudent = await Student.findOne({ srnNo: userParam.srnNo });

    if (checkstudent && checkuserphone && checkstudent.parent == checkuserphone.id) {
      parent = checkuserphone;
    } else if (checkstudent && checkuserphone && checkstudent.parent != checkuserphone.id) {
      let updateStudent = {};
      ...
      if (!checkuserphone) {
        ...
        newUserParam.fullName = userParam.parentFirstName+" "+userParam.parentLastName
      ...
      if (!checkstudent) {   
        ...
        const departments = await Departmentmaster.find({ module: 'sa', name: deptName }).select('id name');
    ...
    if (parent) {
      ...
      const profile_pic = (parent.hasOwnProperty('image') && parent.image != "" && parent.image != null) ? config.repositoryHost + parent.image : config.user_image;
      const is_usertype_exists = (parent.usertype == "") ? false : true
      const is_registered = (parent.firstName == "" || parent.pincode == "") ? false : true
