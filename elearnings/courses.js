...
module.exports = function (app) {
    ...
    app.get("/courses/quizquestions", async function (req, response) {
        let quizid = (!req.query.quiz || req.query.quiz === "undefined") ? "" : req.query.quiz;
        quizid = (quizid) ? quizid : "";
        const questions = await courseService.getQuizQuestions(quizid).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.get("/courses/modules", async function (req, response) {
        ...
        userid = (userid) ? userid : "";
        const modules = await courseService.getCourseModules(userid,course).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/enrollment/cancel", async function (req, response) {
        ...
        const enroll = await courseService.cancelEnrollment(request_body).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/progress_update", async function (req, response) {
        let request_body = req.body;
        const progress = await courseService.progressUpdate(request_body).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/feedback", async function (req, response) {
        let request_body = req.body;
        const feedback = await courseService.saveFeedback(request_body).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.get("/courses/feedbacks", async function (req, response) {
        ...
        userid = (userid) ? userid : "";
        const feedbacks = await courseService.feedbacks(course, userid, pageNo).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/likeafeedback", async function (req, response) {
        let request_body = req.body;
        const feedbacklike = await courseService.likeFeedback(request_body).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/livesessionrequest", async function (req, response) {
        let request_body = req.body;
        const livesession = await courseService.liveSessionRequest(request_body).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/saveQuizResults", async function (req, response) {
        ...
        const quizResults = await courseService.saveQuizResults(requestdata).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/getQuizResults", async function (req, response) {
        let requestdata = req.body;
        const quizResult = await courseService.getQuizResults(requestdata).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
    ...
    app.post("/courses/getCertificate", async function (req, response) {
        let requestdata = req.body;
        const certificate = await courseService.getCertificate(requestdata).catch(e => {
            let status_code = (e.error_status) ? e.error_status : 400
            let message = (e.message) ? e.message : e
            response.status(status_code).send({"message": message});
        });
