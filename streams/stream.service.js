...
async function getAapkiBaithakVideos() {
  let streams = await Stream.find({
    item_type: "video",
    available_for_aapki_baithak: true,
  }).sort({ publishDate: -1 });
  let desc = "";
  let streamList = [];
  for (const cstream of streams) {
    ...
    let streamDetail = await Videostream.findById(cstream["item_id"]).populate(
      "author",
      ["fullName", "is_verified", "image", "location", "badge"]
    );
    if (streamDetail) {
      if (streamDetail.author != "" && streamDetail.author != null) {
        ...
        stream["badge"] = 0;
        stream["image"] =
          streamDetail.author.image != "" && streamDetail.author.image != null
      ...
      stream["available_for_aapki_baithak"] =
        streamDetail.available_for_aapki_baithak;
...
async function getAllold(req) {
  ...
  if (req.body.search.value != "") {
    ...
    let imagesearchresults = await Imagestream.find(querysearch).populate(
      "author",
      ["fullName", "is_verified", "image", "location", "badge", "usertype"]
    );
    let imagestreams = {};
    let imagestreamids = [];
    ...
    let textsearchresults = await Textstream.find(querysearch).populate(
      "author",
      ["fullName", "is_verified", "image", "location", "badge", "usertype"]
    );
    let textstreams = {};
    ...
    let videosearchresults = await Videostream.find(querysearch).populate(
      "author",
      ["fullName", "is_verified", "image", "location", "badge", "usertype"]
    );
    let videostreams = {};
  ...
  for (const cstream of streams) {
    ...
    let streamDetail = "";
    if (cstream["item_type"] === "video") {
      streamDetail = await Videostream.findById(cstream["item_id"]).populate(
        "author",
        ["fullName", "is_verified", "image", "location", "badge"]
      );
    } else if (cstream["item_type"] === "image") {
      streamDetail = await Imagestream.findById(cstream["item_id"]).populate(
        "author",
        ["fullName", "is_verified", "image", "location", "badge"]
      );
    } else if (cstream["item_type"] === "text") {
      streamDetail = await Textstream.findById(cstream["item_id"]).populate(
        "author",
        ["fullName", "is_verified", "image", "location", "badge"]
      );
    } else if (cstream["item_type"] === "sssvideo") {
      streamDetail = await Video.findById(cstream["item_id"])
        .populate("author", [
          "fullName",
          "is_verified",
          "image",
    ...
    if (streamDetail) {
      ...
      if (streamDetail.author) {
        stream["author"] = streamDetail.author.fullName
          ? titleize(streamDetail.author.fullName)
          : "";
        stream["location"] = streamDetail.author.location;
        stream["badge"] =
...
async function search(
  ...
  queryor = {};
  query["$or"] = [];
  searches = await Imagestream.find(querysearch).populate("author", [
    "fullName",
    "is_verified",
    "image",
    "location",
  ...
  searches = await Textstream.find(querysearch).populate("author", [
    "fullName",
    "is_verified",
    "image",
    "location",
  ...
  searches = await Videostream.find(querysearch).populate("author", [
    "fullName",
    "is_verified",
    "image",
    "location",
  ...
  for (const curstream of streams) {
    if (curstream["item_type"] === "sssvideo") {
      let stream = await getSSSVideoStreamDetail(
        curstream,
        curUser,
        sssvideolikes
      );
      if (stream) streamList.push(stream);
    } else {
      let stream = await getStreamDetail(
        curstream,
        curUser,
        videolikes,
        imagelikes,
...
async function getStreamDetail(
  ...
  try {
    ...
    if (curstream["item_type"] === "video") {
      streamDetail = await Videostream.findById(curstream["item_id"]).populate(
        "author",
        ["fullName", "is_verified", "image", "location", "badge", "usertype"]
      );
    } else if (curstream["item_type"] === "image") {
      streamDetail = await Imagestream.findById(curstream["item_id"]).populate(
        "author",
        ["fullName", "is_verified", "image", "location", "badge", "usertype"]
      );
    ...
    if (streamDetail) {
      ...
      //end of if condition checking for the presence of 'streamDetail'
      defer.reject("");
    }
  } catch (e) {
    console.log(e);
...
async function getStreamDetailOther(
  streamsArray,
  curUser,
  videolikes,
  imagelikes,
  ...
  try {
    ...
    let videoStreamDetailsArray = await Videostream.find({
      _id: { $in: videoItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
      "image",
    ...
    let imageStreamDetailsArray = await Imagestream.find({
      _id: { $in: imageItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
      "image",
    ...
    let textStreamDetailsArray = await Textstream.find({
      _id: { $in: textItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
      "image",
    ...
    let vidImgTxtTotalStreamDetails = videoStreamDetailsArray.concat(
      imageStreamDetailsArray
    );
    vidImgTxtTotalStreamDetails = vidImgTxtTotalStreamDetails.concat(
      textStreamDetailsArray
    );

    for (const streamDetail of vidImgTxtTotalStreamDetails) {
      let stream = {};
      let receivedStreamObj = _.where(streamsArray, {
        item_id: streamDetail["id"],
      });
      ...
      if (receivedStreamObj[0]["item_type"] === "video") {
        ...
        if (receivedStreamObj[0]["is_sampark_didi_post"] == false)
          stream["daysToExpiry"] = calculateExpiryDays(
            receivedStreamObj[0]["publishDate"]
          );
        ...
        if (
          streamDetail.reasonToShare != "" &&
          streamDetail.resourceType != "" &&
          streamDetail.resourceLink != "" &&
          streamDetail.reasonToShare != undefined &&
          streamDetail.resourceType != undefined &&
          streamDetail.resourceLink != undefined
        ) {
          ...
          if (streamDetail.resourceType.toLowerCase() === "video") {
            ...
            stream["thumbnail"] =
              ...
              streamDetail.resourceLink +
              "/hqdefault.jpg";
          } else stream["resourceLink"] = streamDetail.resourceLink;
        }
      }
      ...
      if (
        receivedStreamObj[0]["block"] !== undefined &&
        receivedStreamObj[0]["block"] !== null
      )
        stream["block"] = receivedStreamObj[0]["block"];

      if (
        receivedStreamObj[0]["cluster"] !== undefined &&
        receivedStreamObj[0]["cluster"] !== null
      )
        stream["cluster"] = receivedStreamObj[0]["cluster"];

      if (
        receivedStreamObj[0]["diseCode"] !== undefined &&
        receivedStreamObj[0]["diseCode"] !== null
      )
        stream["diseCode"] = receivedStreamObj[0]["diseCode"];

      if (
        receivedStreamObj[0]["pincode"] !== undefined &&
        receivedStreamObj[0]["pincode"] !== null
      )
      ...
      else stream["viewcount"] = receivedStreamObj[0]["viewcount"];

      stream["viewcount"] =
        parseInt(stream["viewcount"]) +
        getAdditionalViewCount(stream["likecount"], stream["commentcount"]);
      ...
      if (receivedStreamObj[0]["item_type"] === "image") {
        stream["thumbnail"] = config.repositoryHost + streamDetail.thumbnail;
        if (
          streamDetail["thumbnails"] != undefined &&
          streamDetail["thumbnails"] != "" &&
          streamDetail["thumbnails"] != null &&
          streamDetail["thumbnails"].length > 0
        ) {
          stream["thumbnails"] = getThumbnailsArray(streamDetail["thumbnails"]);
          stream["thumbnails_dimensions"] =
            streamDetail["thumbnails_dimensions"];
        } else {
          stream["thumbnails"] = [];
          stream["thumbnails"].push(
            config.repositoryHost + streamDetail.thumbnail
          );
        }
        if (
          streamDetail["pdfurl"] != undefined &&
          streamDetail["pdfurl"] != "" &&
          streamDetail["pdfurl"] != null
        )
          stream["pdfurl"] = config.repositoryHost + streamDetail["pdfurl"];
        else stream["pdfurl"] = "";
      } else if (receivedStreamObj[0]["item_type"] === "text") {
        if (
          streamDetail["linkurl"] != undefined &&
          streamDetail["linkurl"] != "" &&
          streamDetail["linkurl"] != null
        )
...
async function getStreamDetailOtherFromBackup(
  streamsArray,
  curUser,
  videolikes,
  imagelikes,
  ...
  try {
    ...
    let videoStreamDetailsArray = await Videostreambackup.find({
      _id: { $in: videoItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
    ...
    let imageStreamDetailsArray = await Imagestreambackup.find({
      _id: { $in: imageItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
    ...
    let textStreamDetailsArray = await Textstreambackup.find({
      _id: { $in: textItemIdArray },
    }).populate("author", [
      "fullName",
      "is_verified",
...
function getAdditionalViewCount(likeCount, commentCount) {
  ...
  let commentViewCount =
    parseInt(commentCount) * config.view_count_multiple_for_comment;
...
async function getSSSVideoStreamDetailForAll(
  streamsArray,
  curUser,
  sssvideolikes
) {
  ...
  try {
    ...
    let streamDetailsArray = await Video.find({ _id: { $in: itemIdArray } })
      .populate("subject", "name")
      .populate("department", "name");
    ...
    for (const streamDetail of streamDetailsArray) {
      ...
      else stream["author"] = "Sampark Didi";
      ...
      stream["is_shareable"] = streamDetail.is_shareable;
      stream["name"] = streamDetail.name;
      stream["description"] = streamDetail.social_content
...
async function getSSSVideoStreamDetail(curstream, curUser, sssvideolikes) {
  let defer = require("q").defer();
  try {
    let streamDetail = await Video.findById(curstream["item_id"])
      .populate("subject", "name")
      .populate("department", "name");
...
async function getAllStateStreams(
  device_id,
  userid,
  pull,
  timestamp,
  ...
  try {
    ...
    let per_page = !timestamp
      ? config.streams_first_page
      : config.streams_per_page; // Calculate number of records to be returned
    ...
    let poststate =
      curUser && curUser.state
        ? curUser && curUser.state.toString()
        : "nostate";
    querystate2["$and"] = [];
    ...
    _.each(streams, function (curstream) {
      if (
        curstream["item_type"] === "image" ||
        curstream["item_type"] === "text"
      )
    ...
    let sssStreamList = await getSSSVideoStreamDetailForAll(
      sssVideoStreams,
      curUser,
      sssvideolikes
    );
    let otherStreamList = await getStreamDetailOther(
      streams,
      curUser,
      videolikes,
      imagelikes,
    ...
    await Videostream.updateMany(
      { _id: { $in: viewVideoList } },
      { $inc: { viewcount: 1 } }
    );
    await Video.updateMany(
      { _id: { $in: viewSSSVideoList } },
      { $inc: { viewcount: 1 } }
    );
    ...
    if (deleteMobileLocalData === 0 && updatetime) {
      ...
      let updated_streams = await Stream.find(query_updated)
        .limit(parseInt(config.max_no_of_streams_on_mobile_storage))
        .sort({ publishDate: -1 });
      ...
      if (sdStreams != []) {
        _.each(sdStreams, function (curstream) {
          let streamObject = _.where(streams, { id: curstream["id"] });
          if (streamObject.length < 1) finalSDStreams.push(curstream);
        });
      }
      ...
      let updatedSssVideoStreams = _.where(updated_streams, {
        item_type: "sssvideo",
      });
      let updatedSssStreamList = await getSSSVideoStreamDetailForAll(
        updatedSssVideoStreams,
        curUser,
        sssvideolikes
      );
      let updatedOtherStreamList = await getStreamDetailOther(
        updated_streams,
        curUser,
        videolikes,
        imagelikes,
    ...
    if (pull == "up") {
      ...
      streamList = _.reject(streamList, function (num) {
        return (
          num.priority == 0 &&
          new Date(num.created_at).getTime() > startOfDay.getTime()
        );
      });
    }
...
async function getUsersStreams(userid) {
  ...
  try {
    ...
    const videoItemIdArray = await fetchItemIds(Videostream, userid, 50);
    const imageItemIdArray = await fetchItemIds(Imagestream, userid, 50);
    // Removing below text stream as not required on mobile
    const textItemIdArray = await fetchItemIds(Textstream, userid, 50);

    const itemIdObjectArray = [...videoItemIdArray, ...imageItemIdArray, ...textItemIdArray];

    const itemIdArray = itemIdObjectArray.map((itemObj) => itemObj["_id"]);

    const streams = await Stream.find({
      item_id: { $in: itemIdArray },
      is_active: true,
    }).sort({ publishDate: -1 });

    let finalStream = [];
    if (streams.length > 0) {
      const curUser = (await getUserDetails(userid, ""))[0];
      finalStream = await getStreamDetailOther(streams, curUser, [], [], []);
    }

    const allFinal = _.sortBy(finalStream, (o) => o.publish_at);
    const newfinal = { streams: allFinal.reverse() };
...
async function fetchItemIds(model, userid, limit) {
  return model
    .find({ author: new objectId(userid) }, { _id: 1 })
    .sort({ createdDate: -1 })
    .limit(limit);
}
...
async function getUsersStreamsFromBackup(userid) {
  ...
  try {
    ...
    if (userid != "" && userid != undefined && userid != null) {
      let videoItemIdArray = await Videostreambackup.find(
        { author: new objectId(userid) },
        { _id: 1 }
      )
        .sort({ createdDate: -1 })
        .limit(50);
      let imageItemIdArray = await Imagestreambackup.find(
        { author: new objectId(userid) },
        { _id: 1 }
      )
        .sort({ createdDate: -1 })
      ...
      // Removing below text stream as not required on mobile
      let textItemIdArray = await Textstreambackup.find(
        { author: new objectId(userid) },
        { _id: 1 }
      )
        .sort({ createdDate: -1 })
      ...
      if (itemIdObjectArray.length > 0) {
        ...
        let streams = await Streambackup.find({
          item_id: { $in: itemIdArray },
          is_active: true,
        }).sort({ publishDate: -1 });
        ...
        if (streams.length > 0) {
          ...
          finalStream = await getStreamDetailOtherFromBackup(
            streams,
            curUser,
            [],
            [],
...
// getMyDistrictStreams function returns District Streams for user
// getMyDistrictStreams starts
async function getMyDistrictStreams(
  device_id,
  userid,
  pull,
  timestamp,
  ...
  try {
    ...
    if (deleteMobileLocalData === 0 && updatetime) {
      ...
      const updated_streams = await Stream.find(query_updated)
        .limit(parseInt(config.max_no_of_streams_on_mobile_storage))
        .sort({ publishDate: -1 });
...
async function getSamparkDidiStreams(
  device_id,
  userid,
  pull,
  timestamp,
...
// getMyDistrictStreams function returns District Streams for user
// getSamparkDidiStreams starts
async function getPodcastStreams(
  device_id,
  userid,
  pull,
  timestamp,
  ...
  try {
    ...
    if (deleteMobileLocalData === 0 && updatetime) {
      ...
      const updated_streams = await Stream.find(query_updated).limit(
        parseInt(config.max_no_of_streams_on_mobile_storage)
      );
...
// getMyDistrictStreams function returns District Streams for user
// getMyDistrictStreams starts
async function getMyStateStreams(
  device_id,
  userid,
  pull,
  timestamp,
  ...
  try {
    ...
    _.each(updatedStreamList, (stream) => {
      if (
        stream["district"] &&
        curUser.district &&
        stream["district"] == curUser.district
      ) {
        stream["priority"] = 0.5;
      }
      if (
      ...
      if (
        stream["cluster"] &&
        curUser.cluster &&
        stream["cluster"] == curUser.cluster
      ) {
        stream["priority"] = 0.3;
      }
      if (
        stream["pincode"] &&
        curUser.pincode &&
        stream["pincode"] == curUser.pincode
      ) {
        stream["priority"] = 0.2;
      }
      if (
        stream["diseCode"] &&
        curUser.diseCode &&
        stream["diseCode"] == curUser.diseCode
      ) {
        stream["priority"] = 0.1;
      }
    });
...
async function getAllStreams(
  ...
  try {
    const query = {
      is_active: true,
      available_for_aapki_baithak: false,
    ...
    if (pull && pull !== "all") {
      const pullCondition = pull === "down" ? "$gt" : "$lt";
      query.publishDate[pullCondition] = timestamp;
    }

    const curUser = (await getUserDetails(userid, social_hours))[0];

    if (curUser && social_hours) {
      await update_social_hours(curUser, social_hours);
    }

    if (stream_for === "mystate") {
      const querystate1 = {
        states: curUser && curUser.state ? curUser.state.toString() : "nostate",
      };
      query["$or"] = [querystate1];
      query_updated["$or"] = [querystate1];
    } else if (stream_for === "mydistrict") {
      if (curUser && curUser.district) {
        const querydistrict1 = { district: curUser.district };
        query["$or"] = [querydistrict1];
        query_updated["$or"] = [querydistrict1];
      } else {
        query.state = "nostate";
        query_updated.state = "nostate";
        query_updated.is_sampark_didi_post = true;
    ...
    const final_query_update = buildFinalQuery(query_updated);
    const final_query = buildFinalQuery(query);
    ...
    if (pull === "all") {
      ...
      const totalstreamcount = await Stream.countDocuments(final_query);

      if (per_page > 0) {
        if (pull === "down" && apkversion >= 4.8) {
          if (totalstreamcount > per_page) {
            deleteMobileLocalData = 1;
            per_page = config.streams_first_page;
          }
        } else {
          reverse_array = pull === "down" ? 1 : 0;
          streams = await Stream.find(final_query)
            .sort({ publishDate: pull === "down" ? 1 : -1 })
            .limit(per_page);
        }
      } else {
        streams = await Stream.find(final_query).sort({ publishDate: -1 });
    ...
    const { sssvideolikes, videolikes, imagelikes, textlikes } =
      await findLikes(userid, device_id);
    ...
    if (deleteMobileLocalData === 0 && updatetime) {
      const updatedStreamIds = updatedStreamList.map((stream) => stream.id);
      const inactiveStreamIds = await Stream.distinct(
        "_id",
        query_inactive
      ...
      const updatedStreamsDetails = await processStreams(
        updated_streams,
        curUser,
        sssvideolikes,
        videolikes,
      ...
      updatedStreamList = updatedStreamsDetails.streamList;
      inactiveStreamList = updatedStreamsDetails.inactiveStreamList;
    }

    const user_streams = {
      streams: reverse_array === 1 ? streamList.reverse() : streamList,
      max_records: config.max_no_of_streams_on_mobile_storage,
      inactivestreams: inactiveStreamList,
      updatedstreams: updatedStreamList,
      delete_mobile_local_data: deleteMobileLocalData,
    };
...
function buildFinalQuery(queryObject) {
...
async function findLikes(userid, device_id) {
  const likequery = userid ? { user: userid } : { device_id: device_id };
  const sssvideolikes = (await Sssvideolike.distinct("video", likequery)).map(
    (item) => item.toString()
  );
  const videolikes = (await Videolike.distinct("video", likequery)).map(
    (item) => item.toString()
  );
  const imagelikes = (await Imagelike.distinct("image", likequery)).map(
    (item) => item.toString()
  );
  const textlikes = (await Textlike.distinct("text", likequery)).map((item) =>
    item.toString()
  );
  return { sssvideolikes, videolikes, imagelikes, textlikes };
}
...
async function processStreams(
  ...
  for (const curstream of streams) {
    ...
    if (curstream.item_type === "sssvideo") {
      const stream = await getSSSVideoStreamDetail(
        curstream,
        curUser,
        sssvideolikes
      );
      ...
      const stream = await getStreamDetail(
        curstream,
        curUser,
        videolikes,
        imagelikes,
...
async function create(req) {
  ...
  if (streamType === "video") {
    ...
    videoParam.available_for_aapki_baithak =
      streamParam.available_for_aapki_baithak;
  ...
  let streamData = {};
  if (curAuthorDistrict !== "") streamData.district = curAuthorDistrict;
  if (curAuthorLocation !== "") streamData.location = curAuthorLocation;
  ...
  try {
    if (await streamDetail.save()) {
      ...
      if (streamType === "video") {
        ...
        streamData.available_for_aapki_baithak =
          streamParam.available_for_aapki_baithak;
      } else {
...
async function edit(id) {
  ...
  if (stream["item_type"] === "video") {
    ...
    streamdetail["fullurl"] =
      config.repositoryHost + streamDetail["youtube_code"];
  ...
  if (stream["item_type"] === "text") streamdetail["thumbnail"] = "";
  else
...
async function update(id, req) {
  ...
  if (await streamDetail.save()) {
    ...
    if (stream["item_type"] === "video") {
      stream_param["available_for_aapki_baithak"] =
        streamParam.available_for_aapki_baithak;
    } else {
    ...
    if (req.files && Object.keys(req.files).length === 1) {
      ...
      let uploadData = await uploadToS3(
        thumbnail,
        "stream_uploads/" + stream["item_type"]
      );
...
async function getAll(req) {
  ...
  let sort_order =
    requestParam.sortorder && requestParam.sortorder === "1" ? "asc" : "desc";
  ...
  for (const column of columns) {
    ...
    if (column.name == "creationdate" && column.search) {
      ...
      querysearchand["createdDate"]["$gte"] = momentObjFrom
        .startOf("day")
        .toISOString();
      querysearchand["createdDate"]["$lte"] = momentObjTo
        .endOf("day")
        .toISOString();
      query["$and"].push(querysearchand);
    }
  ...
  return_data["end"] =
    streamList.length < perPage ? parseInt(start) + streamList.length : end;
  if (filteredstreams % perPage === 0)
    return_data["totalPages"] = parseInt(filteredstreams / perPage);
  else return_data["totalPages"] = parseInt(filteredstreams / perPage) + 1;
  if (pageNo === 1) {
...
async function getmyClassroomVideos(
  userid,
  pull,
  timestamp,
  updatetime,
  ...
  try {
    ...
    const teacher = await User.findOne(
      { _id: userid, usertype: "govt teacher" },
      { diseCode: 1 }
    ); // find disecode
    ...
    if (teacher) {
      ...
      let allTeachers = await User.find(
        ...
        { _id: 1 }
      ); // find all teachers for this disecode
      ...
      if (allTeachers) {
        ...
        if (per_page > 0) {
          if (pull === "down") {
            ...
            if (apkversion < 4.8) {
              reverse_array = 1;
              videoStreamDetails = await Videostream.find(videoQuery)
                .populate("author", [
                  "fullName",
                  "is_verified",
                  "image",
        ...
        if (videoStreamDetails) {
          ...
          for (const streamDetail of videoStreamDetails) {
            let stream = {};
            checkLiked = await Videolike.find({
              video: streamDetail._id,
              user: userid,
            });

            //code to update view count of streams
            await Videostream.updateOne(
              { _id: streamDetail._id },
              { $inc: { viewcount: 1 } }
            );
