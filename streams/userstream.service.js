...
async function getLocation(stream, streamDetail, curUser) {  
  author = (streamDetail.author !== undefined && streamDetail.author !== null) ? streamDetail.author.id : "";
  ...
  if (!usertype || (usertype && usertype == "admin")) {
    if (curUser == "") {
      return (author == "") ? "India" : author.location;
    } else {
      return curUser;
...
async function getAllStreams(device_id,userid,pull,timestamp,social_hours,updatetime,stream_for) {
    var query = {};
    var query_updated = {};
    var query_inactive = {};
    if (timestamp === "")
      var per_page = config.streams_first_page;
    else
      var per_page = config.streams_per_page;
...
//        query_updated["states"] = "others";
        for(i=0; i<config.states.length; i++) {
          if (config.states[i].id == curUserState && config.states[i].is_active == "true") {
    ...
    if (pull == "all") {
      ...
      if (per_page > 0) {
        streams = await Stream.find(query).sort({publishDate: -1}).limit(per_page);
      } else {
        streams = await Stream.find(query).sort({publishDate: -1});
      }
    }
    ...
    var textlikes = {};
    if (userid !== "") {
      usersssvideolikes = await Sssvideolike.find({user: userid}).select('video is_liked');
      for(var i = 0; i < usersssvideolikes.length; i++){
        sssvideolikes[usersssvideolikes[i]["video"]] = usersssvideolikes[i]["is_liked"];
      }
      uservideolikes = await Videolike.find({user: userid}).select('video is_liked');
      for(var i = 0; i < uservideolikes.length; i++){
        videolikes[uservideolikes[i]["video"]] = uservideolikes[i]["is_liked"];
      }
      userimagelikes = await Imagelike.find({user: userid}).select('image is_liked');
      for(var i = 0; i < userimagelikes.length; i++){
        imagelikes[userimagelikes[i]["image"]] = userimagelikes[i]["is_liked"];
      }
      usertextlikes = await Textlike.find({user: userid}).select('text is_liked');
      for(var i = 0; i < usertextlikes.length; i++){
        textlikes[usertextlikes[i]["text"]] = usertextlikes[i]["is_liked"];
      }
    } else {
      usersssvideolikes = await Sssvideolike.find({device_id: device_id}).select('video is_liked');
      for(var i = 0; i < usersssvideolikes.length; i++){
        sssvideolikes[usersssvideolikes[i]["video"]] = usersssvideolikes[i]["is_liked"];
      }
      uservideolikes = await Videolike.find({device_id: device_id}).select('video is_liked');
      for(var i = 0; i < uservideolikes.length; i++){
        videolikes[uservideolikes[i]["video"]] = uservideolikes[i]["is_liked"];
      }
      userimagelikes = await Imagelike.find({device_id: device_id}).select('image is_liked');
      for(var i = 0; i < userimagelikes.length; i++){
        imagelikes[userimagelikes[i]["image"]] = userimagelikes[i]["is_liked"];
      }
      usertextlikes = await Textlike.find({device_id: device_id}).select('text is_liked');
      for(var i = 0; i < usertextlikes.length; i++){
        textlikes[usertextlikes[i]["text"]] = usertextlikes[i]["is_liked"];
      }
    }
    ...
    for(const curstream of streams) {
      ...
      if (curstream['item_type'] == "sssvideo") {
        streamDetail = await Video.findById(curstream['item_id']).populate('subject','name').populate('department','name');        
        if (streamDetail) {          
          stream = {};
          ...
          if(sssvideolikes.hasOwnProperty(curstream["item_id"])){
            stream["is_liked"] = (sssvideolikes[curstream["item_id"]]) ? sssvideolikes[curstream["item_id"]] : "";
          } else {
            stream["is_liked"] = "";
          }
          ...
          if (streamDetail.author != "" && streamDetail.author != null) {
            if (streamDetail.author && streamDetail.author.firstName) {
              stream['author'] = (streamDetail.author.lastName) ? titleize(streamDetail.author.firstName+" "+streamDetail.author.lastName) : titleize(streamDetail.author.firstName);
            } else {
              stream['author'] = "Sampark Didi";
            }
        ...
        if (curstream['item_type'] == "video") {
          streamDetail = await Videostream.findById(curstream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);

        } else if (curstream['item_type'] == "image") {
          streamDetail = await Imagestream.findById(curstream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);
          
        } else if (curstream['item_type'] == "text") {
          streamDetail = await Textstream.findById(curstream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);        
        } else if (curstream['item_type'] == "sssvideo") {
          streamDetail = await Video.findById(curstream['item_id']).populate('subject','name').populate('department','name');
        } else if (curstream['item_type'] == "sssaudio") {
          streamDetail = await Audio.findById(curstream['item_id']).populate('subject','name').populate('department','name');
        }
        if (streamDetail) {
          stream = {};
          if (curstream['item_type'] == "video") {
            if(videolikes.hasOwnProperty(curstream["item_id"])){
              stream["is_liked"] = (videolikes[curstream["item_id"]]) ? videolikes[curstream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            ...
            if(imagelikes.hasOwnProperty(curstream["item_id"])){
              stream["is_liked"] = (imagelikes[curstream["item_id"]]) ? imagelikes[curstream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            stream['module'] = "baithak"
            ...
            if(textlikes.hasOwnProperty(curstream["item_id"])){
              stream["is_liked"] = (textlikes[curstream["item_id"]]) ? textlikes[curstream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            stream['module'] = "baithak"
    ...
    if (updatetime !== "") {
      ...
      for(const curustream of updated_streams) {
        if (curustream['item_type'] == "video") {
          streamDetail = await Videostream.findById(curustream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);
        } else if (curustream['item_type'] == "image") {
          streamDetail = await Imagestream.findById(curustream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);
        } else if (curustream['item_type'] == "text") {
          streamDetail = await Textstream.findById(curustream['item_id']).populate('author',['firstName','lastName', 'is_verified', 'image', 'location', 'badge', 'usertype']);
        } else if (curustream['item_type'] == "sssvideo") {
          streamDetail = await Video.findById(curustream['item_id']).populate('subject','name').populate('department','name');
        }
        ...
        if (streamDetail) {
          ...
          if (curustream['item_type'] == "video") {
            if(videolikes.hasOwnProperty(curustream["item_id"])){
              stream["is_liked"] = (videolikes[curustream["item_id"]]) ? videolikes[curustream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            stream['module'] = "baithak"
            ...
            if(imagelikes.hasOwnProperty(curustream["item_id"])){
              stream["is_liked"] = (imagelikes[curustream["item_id"]]) ? imagelikes[curustream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            stream['module'] = "baithak"
            ...
            if(textlikes.hasOwnProperty(curustream["item_id"])){
              stream["is_liked"] = (textlikes[curustream["item_id"]]) ? textlikes[curustream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
            stream['module'] = "baithak"
            ...
            if(sssvideolikes.hasOwnProperty(curustream["item_id"])){
              stream["is_liked"] = (sssvideolikes[curustream["item_id"]]) ? sssvideolikes[curustream["item_id"]] : "";
            } else {
              stream["is_liked"] = "";
            }
          }
          ...
          if (streamDetail.author != "" && streamDetail.author != null) {
            ...
            stream['location'] = await getLocation(curustream, streamDetail, curUser);
            stream['badge'] = (!streamDetail.author.badge || streamDetail.author.badge == "" || streamDetail.author.badge == "null") ? 0 : streamDetail.author.badge;
            if (stream['author'] == "Sampark Didi") {
              stream['image'] = config.didi_image;
            } else {
