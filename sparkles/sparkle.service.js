	...
	if (teachers && teachers.length) {
		...
		for (let i = 0; i < teachers.length; i++) {
			...
			if (userSparkleData && userSparkleData.length) {
				userSparkleData[0]["sparkvisit"] = userSparkleData[0]["sparkvisit"] ? (parseInt(userSparkleData[0]["sparkvisit"]) + finalTotal) : finalTotal;
				userSparkleData[0]["total"] = userSparkleData[0]["total"] ? (parseInt(userSparkleData[0]["total"]) + finalTotal) : finalTotal;
...
async function topTeachers(userParam) {
	...
	//console.log("sessionStartYear 4 : ",sessionStartYear)
		...
		if (cntTopTeachers > 0) {
			let limitNumber = (cntTopTeachers > 10) ? parseInt((cntTopTeachers * 10) / 100) : cntTopTeachers;

			//To get all block names in this district
			const dbBlocks = await Block.find({ "district_id": userdistrict, "is_active": true }).select('id name');
			...
			if (sortOrder == 'ascending') {
				dbTopTeachers = _.sortBy(dbTopTeachers, item => item.fullName.toLowerCase());
			} else if (sortOrder == 'descending') {
				dbTopTeachers = _.sortBy(dbTopTeachers, item => item.fullName.toLowerCase()).reverse();
			}
			...
			if (dbTopTeachers && dbTopTeachers.length) {
				for (var i = 0; i < dbTopTeachers.length; i++) {
					...
					teacher.image = (dbTopTeachers[i]["image"] != "" && dbTopTeachers[i]["image"] != null) ? config.repositoryHost + dbTopTeachers[i]["image"] : config.user_image;
					...
					if (dbTopTeachers[i].block != null) {
						blockId = dbTopTeachers[i]["block"] ? dbTopTeachers[i]["block"].toString() : dbTopTeachers[i]["block"];
...
async function topSchools(userParam) {
	...
	try {
		...
		let endMonth = parseInt(userParam.endMonth);
		let sortOrder = userParam.sortOrder != undefined ? userParam.sortOrder : "ascending";
		let query = {};
		let queryCurrent = {};
		let queryNext = {};
		let queryCurrentYear = {};
		...
		const dbSchools = await School.find({ "district": userdistrict, "isActive": true }).select('diseCode schoolName');
		...
		if (dbTopSchools && dbTopSchools.length) {
			let limitNumber = (dbTopSchools.length > 20) ? parseInt((dbTopSchools.length * 20) / 100) : dbTopSchools.length;
			...
			for (var i = 0; i < dbTopSchools.length; i++) {
				...
				//To get school name
				let schoolName = _.where(dbSchools, { diseCode: dbTopSchools[i]["_id"]["diseCode"] });
		...
		if (sortOrder == 'ascending') {
			...
			records = _.sortBy(schools, item => item.schoolName.toLowerCase()).reverse();
		}
...
async function topBlocks(userParam) {
	...
	const dbBlocks = await Block.find({ "state_id": userstate, "is_active": true }).select('id name');
	...
	if (dbTopBlocks && dbTopBlocks.length) {
		let limitNumber = (dbTopBlocks.length > 20) ? parseInt((dbTopBlocks.length * 20) / 100) : dbTopBlocks.length;
		for (var i = 0; i < dbTopBlocks.length; i++) {
			block = {};
			blockId = dbTopBlocks[i]["_id"]["block"] ? dbTopBlocks[i]["_id"]["block"].toString() : dbTopBlocks[i]["_id"]["block"];
	...
	if (sortOrder == 'ascending') {
		...
		records = _.sortBy(blocks, item => item.blockName.toLowerCase()).reverse();
	}
	return records;
}
...
async function trainingCompletionSparkleUpdate(userParam) {
	...
	if (teachers == 0) {
		...
		if (userSparkleData && userSparkleData.length) {
			userSparkleData[0]["training"] = userSparkleData[0]["training"] ? (parseInt(userSparkleData[0]["training"]) + points) : points;
			userSparkleData[0]["total"] = userSparkleData[0]["total"] ? (parseInt(userSparkleData[0]["total"]) + points) : points;
			userSparkleData[0]["updatedDate"] = toDateObject;
			let useSparkleObj = new Sparkle(userSparkleData[0]);
			await useSparkleObj.save();
		}
...
async function getMySparkles(userParam) {
	...
	try {
		...
		const district = (userParam.district != "") ? new objectId(userParam.district) : "";
		let topPercentage = "";
		let nextAchievement = {};
		let collectionName;
		...
		let TeacherranksData = await collectionName.find({ user: user, month: currentMonth, year: currentYear }, { rank: 1, grandTotal: 1 });
		...
		const getSessionData = await cronmethods.getSessionData(currentYear, currentMonth, user);
			...
			[
				...
				},
				{
					$project: {
						...
						sssTotal: "$sssTotal",
						baithakTotal: { "$add": ["$baithakLikesTotal", "$baithakCommentsTotal", "$postFromSSS"] },
						courseTotal: "$courseTotal",
						assessmentTotal: "$assessmentTotal",
						digitalDiaryTotal: "$digitalDiaryTotal",
						bonusSpakleTotal: { "$add": ["$trainingTotal", "$sparkvisitTotal", "$surveyTotal"] },
						stv: "$stv",
					}
				}
			]
		...
		if (userSparkleData && userSparkleData.length > 0) {
			...
			mySparkle['sss']['topic'] = (userSparkleData[0].sssTotal / config.sparkle_points_sss);
			mySparkle['sss']['sparkles'] = userSparkleData[0].sssTotal;
			mySparkle['sss']['top'] = await getModuleTopPercentage(user, 'sss', userType);
	
			mySparkle['baithak']['topic'] = userSparkleData[0].baithakTotal;
			mySparkle['baithak']['sparkles'] = userSparkleData[0].baithakTotal;
			mySparkle['baithak']['top'] = await getModuleTopPercentage(user, 'baithak', userType);
	
			mySparkle['courses']['topic'] = (userSparkleData[0].courseTotal / config.sparkle_points_elearning);
			mySparkle['courses']['sparkles'] = userSparkleData[0].courseTotal;
			mySparkle['courses']['top'] = await getModuleTopPercentage(user, 'courses', userType);
	
			mySparkle['didiksawal']['topic'] = (userSparkleData[0].assessmentTotal / config.sparkle_points_assessment);
			mySparkle['didiksawal']['sparkles'] = userSparkleData[0].assessmentTotal;
			mySparkle['didiksawal']['top'] = await getModuleTopPercentage(user, 'assessment', userType);
	
			mySparkle['digitalDiary']['topic'] = (userSparkleData[0].digitalDiaryTotal / config.sparkle_points_digitaldiaryplandone);
			mySparkle['digitalDiary']['sparkles'] = userSparkleData[0].digitalDiaryTotal;
			mySparkle['digitalDiary']['top'] = await getModuleTopPercentage(user, 'digitaldiary', userType);
	
			mySparkle['bonusSparkle']['topic'] = (userSparkleData[0].bonusSpakleTotal);
			mySparkle['bonusSparkle']['sparkles'] = userSparkleData[0].bonusSpakleTotal;
			mySparkle['bonusSparkle']['top'] = await getModuleTopPercentage(user, 'bonus', userType);

			mySparkle['stv']['topic'] = userSparkleData[0].stv / config.sparkle_points_stv;
			mySparkle['stv']['sparkles'] = userSparkleData[0].stv;
			mySparkle['stv']['top'] = await getModuleTopPercentage(user, 'stv', userType);
		}
...
async function getModuleTopPercentage(userId, module, userType) {
	...
	try {
		...
		if (module == 'baithak') {
			let baithaklikes = await baithakCollationName.find({ user: userId }, { baithaklikesRank: 1 });
			if (baithaklikes.length && baithaklikes[0].baithaklikesRank != undefined) {
				moduleRank = Math.round(baithaklikes[0].baithaklikesRank);
			}
		}
		if (module == 'courses') {
			let courses = await courseCollectionName.find({ user: userId }, { courseRank: 1 });
			if (courses.length && courses[0].courseRank != undefined) {
				moduleRank = Math.round(courses[0].courseRank);
			}
		}
		if (module == 'assessment') {
			let assessments = await assessmentCollectionName.find({ user: userId }, { assessmentRank: 1 });
		...
		if (module == 'digitaldiary') {
			let digitaldiary = await digitalDiaryCollectionName.find({ user: userId }, { digitalDiaryRank: 1 });
			if (digitaldiary.length && digitaldiary[0].digitalDiaryRank != undefined) {
				moduleRank = Math.round(digitaldiary[0].digitalDiaryRank);
			}
		}
		if (module == 'bonus') {
			let bonus = await bonusCollectionName.find({ user: userId }, { bonusRank: 1 });
...
async function getSparkleDifference(range, district, currentgrandTotal, userType) {
	...
	try {
		...
		let getAboveRange = await collectionName.find({ month: currentMonth, year: currentYear, district: district, rank: { "$lt": range } }, { grandTotal: 1 }).sort({ grandTotal: -1 }).limit(1);
...
async function getMySchoolSparkles(userParam) {
	...
	let diseCode = userParam.diseCode;
	let mySchoolSparkles = [];
	let mySchoolSparkle = {};
	let finalArr = [];
	mySchoolSparkle['grandTotal'] = 0;
	mySchoolSparkle['sss'] = {};
	mySchoolSparkle['baithak'] = {};
	mySchoolSparkle['courses'] = {};
	mySchoolSparkle['didiksawal'] = {};
	mySchoolSparkle['digitalDiary'] = {};
	mySchoolSparkle['bonusSparkle'] = {};
	mySchoolSparkle['stv'] = {};
	let totalSparkles = 0;
	let totalsssSparkles = 0;
	let baithakLikes = 0;
	let baithakComments = 0;
	let totalbaithakSparkles = 0;
	let totalcourseSparkles = 0;
	let totaldidiksawalSparkles = 0;
	let totaldigitalDiarySparkles = 0;
	let totalbonusSparkles = 0;
	let totalStvSparkles = 0;
	let training = 0;
	let sparkvisit = 0;
	let survey = 0;

	let schoolUsers = await User.aggregate([
		{
			$match: {
				diseCode: diseCode
			}
	...
	if (schoolUsers && schoolUsers.length > 0) {
		for (let y = 0; y < schoolUsers.length; y++) {
			newarr = {};
			newarr = schoolUsers[y]._id;
			finalArr.push(newarr);
		}
	}

	let schoolUsersSparkles = await Sparkle.find({ user: { $in: finalArr } });

	if (schoolUsersSparkles && schoolUsersSparkles.length > 0) {
		for (m = 0; m < schoolUsersSparkles.length; m++) {

			if (schoolUsersSparkles[m].total != undefined) {
				totalSparkles = totalSparkles + schoolUsersSparkles[m].total;
			}
			if (schoolUsersSparkles[m].sss != undefined) {
				totalsssSparkles = totalsssSparkles + schoolUsersSparkles[m].sss;
			}
			if (schoolUsersSparkles[m].baithakLikes != undefined) {
				baithakLikes = baithakLikes + schoolUsersSparkles[m].baithakLikes;
			}
			if (schoolUsersSparkles[m].baithakComments != undefined) {
				baithakComments = baithakComments + schoolUsersSparkles[m].baithakComments;
			}
			if (schoolUsersSparkles[m].course != undefined) {
				totalcourseSparkles = totalcourseSparkles + schoolUsersSparkles[m].course;
			}
			if (schoolUsersSparkles[m].assessment != undefined) {
				totaldidiksawalSparkles = totaldidiksawalSparkles + schoolUsersSparkles[m].assessment;
			}
			if (schoolUsersSparkles[m].digitalDiary != undefined) {
				totaldigitalDiarySparkles = totaldigitalDiarySparkles + schoolUsersSparkles[m].digitalDiary;
			}
			if (schoolUsersSparkles[m].training != undefined) {
				training = training + schoolUsersSparkles[m].training;
			}
			if (schoolUsersSparkles[m].sparkvisit != undefined) {
				sparkvisit = sparkvisit + schoolUsersSparkles[m].sparkvisit;
			}
			if (schoolUsersSparkles[m].survey != undefined) {
				survey = survey + schoolUsersSparkles[m].survey;
			}
			if (schoolUsersSparkles[m].stv != undefined) {
				totalStvSparkles = totalStvSparkles + schoolUsersSparkles[m].stv;
			}
		}
	}

	totalbaithakSparkles = baithakLikes + baithakComments;
	totalbonusSparkles = training + sparkvisit + survey;

	mySchoolSparkle['grandTotal'] = totalSparkles;
	mySchoolSparkle['sss']['topic'] = (totalsssSparkles / config.sparkle_points_sss);
	mySchoolSparkle['sss']['sparkles'] = totalsssSparkles;
	mySchoolSparkle['baithak']['topic'] = totalbaithakSparkles;
	mySchoolSparkle['baithak']['sparkles'] = totalbaithakSparkles;
	mySchoolSparkle['courses']['topic'] = (totalcourseSparkles / config.sparkle_points_elearning);
	mySchoolSparkle['courses']['sparkles'] = totalcourseSparkles;
	mySchoolSparkle['didiksawal']['topic'] = (totaldidiksawalSparkles / config.sparkle_points_assessment);
	mySchoolSparkle['didiksawal']['sparkles'] = totaldidiksawalSparkles;
	mySchoolSparkle['digitalDiary']['topic'] = (totaldigitalDiarySparkles / config.sparkle_points_digitaldiaryplandone);
	mySchoolSparkle['digitalDiary']['sparkles'] = totaldigitalDiarySparkles;
	mySchoolSparkle['bonusSparkle']['topic'] = totalbonusSparkles;
	mySchoolSparkle['bonusSparkle']['sparkles'] = totalbonusSparkles;
	mySchoolSparkle['stv']['topic'] = (totalStvSparkles / config.sparkle_points_stv);
	mySchoolSparkle['stv']['sparkles'] = totalStvSparkles;
...
async function getDistrictWiseTeachersDataToShareinExcel(userParam) {
	...
	const AllDistricts = await District.find({}, { '_id': 1, 'name': 1 }).sort({ 'name': 1 });
	...
	if (AllDistricts && AllDistricts.length > 0) {
		for (let y = 0; y < AllDistricts.length; y++) {
			...
			userRankData = await customPagination(newarr, pageNumber, NumberOfRecordsPerPage);
			...
			if (userRankData.total > 0) {
				...
				for (let j = 0; j < userRankData.length; j++) {
					...
					let districtData = await District.findOne({ _id: districtId }, { state_id: 1, _id: 0 });

					if (districtData) {
						stateData = await State.findOne({ _id: districtData.state_id }, { name: 1, _id: 0 })
					...
					let sssQuery = await TeachersssRank.findOne({ user: userId }, { sssTotal: 1, _id: 0 });
					if (sssQuery) {
						sssTotal = sssQuery.sssTotal;
						temp['sss'] = sssTotal;
					}

					let baithakLikesQuery = await TeacherbaithaklikesRank.findOne({ user: userId }, { baithaklikesTotal: 1, _id: 0 });
					...
					if (baithakLikesQuery) {
						...
						temp['baithakLikesComments'] = baithakLikesTotal + baithakCommentsTotal;
					}

					let courseQuery = await TeachercourseRank.findOne({ user: userId }, { courseTotal: 1, _id: 0 });
					if (courseQuery) {
						courseTotal = courseQuery.courseTotal;
						temp['course'] = courseTotal;
					}

					let assessmentQuery = await TeacherassessmentRank.findOne({ user: userId }, { assessmentTotal: 1, _id: 0 });
					if (assessmentQuery) {
						assessmentTotal = assessmentQuery.assessmentTotal;
						temp['assessment'] = assessmentTotal;
					}

					let digitalDiaryQuery = await TeacherdigitaldiaryRank.findOne({ user: userId }, { digitalDiaryTotal: 1, _id: 0 });
					if (digitalDiaryQuery) {
						digitalDiaryTotal = digitalDiaryQuery.digitalDiaryTotal;
						temp['digitalDiary'] = digitalDiaryTotal;
					}

					let bonusQuery = await TeacherbonusRank.findOne({ user: userId }, { bonusTotal: 1, _id: 0 });
					if (bonusQuery) {
...
async function topTeachersByState(userParam) {
	...
	//console.log("sessionStartYear 4 : ",sessionStartYear)
		...
		if (cntTopTeachers > 0) {
			...
			const dbBlocks = await Block.find({ "state_id": userstate, "is_active": true }).select('id name');
			const dbDistricts = await District.find({ "state_id": userstate, "is_active": true }).select('id name');
			...
			if (dbTopTeachers && dbTopTeachers.length) {
				for (var i = 0; i < dbTopTeachers.length; i++) {
					...
					if (dbTopTeachers[i].district != null) {
						districtId = dbTopTeachers[i]["district"] ? dbTopTeachers[i]["district"].toString() : dbTopTeachers[i]["district"];
...
async function topSchoolsByState(userParam) {
	...
	try {
		...
		const dbSchools = await School.find({ "state": userstate, "isActive": true }).select('diseCode schoolName');
		...
		if (dbTopSchools && dbTopSchools.length) {
			...
			for (var i = 0; i < dbTopSchools.length; i++) {
				...
				let districtName = ""
				//To get school name
				let schoolName = _.where(dbSchools, { diseCode: dbTopSchools[i]["school"]["diseCode"] });

				if (dbTopSchools[i]["block"]) {
					blockName = _.where(dbBlocks, { id: dbTopSchools[i]["block"].toString() });
				}
				if (dbTopSchools[i]["district"]) {
					districtName = _.where(dbDistricts, { id: dbTopSchools[i]["district"].toString() });
				}
...
async function topDistrictsByState(userParam) {
	...
	if (dbTopDistricts && dbTopDistricts.length) {
		let limitNumber = (dbTopDistricts.length > 20) ? parseInt((dbTopDistricts.length * 20) / 100) : dbTopDistricts.length;
		for (var i = 0; i < dbTopDistricts.length; i++) {
			district = {};
			districtId = dbTopDistricts[i]["_id"]["district"] ? dbTopDistricts[i]["_id"]["district"].toString() : dbTopDistricts[i]["_id"]["district"];
	...
	if (sortOrder == 'ascending') {
		...
		records = _.sortBy(districts, item => item.districtName.toLowerCase()).reverse();
	} else {
		records = districts
	}
	return records;
